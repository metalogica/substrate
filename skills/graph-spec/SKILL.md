...
1. **Read the manifest** at `docs/doctrine/doctrine-manifest.yaml`. Take each entry's `id` and its
   ...
   accommodate it.
2. **Silent no-op when there is nothing to bind.** No manifest, or no entry declares `paths:` →
...
Worked example, against this repo's manifest (`agents` → `paths: [docs/doctrine/**,
