...
4. Dispatch **one group-runner (`bead-implementer`) via the Task tool** (one per window, in a single message; sequential fallback if serialized) with, inlined:
   ...
   - **the window's doctrine digests — push, don't point.** Union the `doctrine:<id>` labels across the window's beads (stamped by `/substrate/graph-spec` Step 4.55 from write-scope ∩ the manifest's `paths:`), **dedupe by `<id>`**, and inline `bash docs/scripts/doctrine-digest.sh <id>` — that doctrine's Binding Rules block — **once for the window**, not once per tuple, each block labelled with its id and the beads it binds. A pointer verifies nothing; a pushed digest provably arrived (`docs/doctrine/agents-doctrine.md` §2.1). **Escalate to the full doctrine body** when a *single* bead's write-scope majority falls inside one doctrine's `paths:`, inlining that whole file for its tuple in place of the digest. Bound it:
     - **At most one escalation per window** (the doctrine covering the most write-scope): a full body costs ~6–10× its digest, and beads whose majorities land in *different* doctrines mean the window is mis-partitioned, not that it needs two full bodies.
     - **Never truncate a digest, never drop one to fit** — half-delivered binding rules are worse than a pointer. Outsized union (≳200 lines) → drop the *escalation* to digest-only; still outsized → the window is bound too widely, narrate it and consider re-batching per Step 2.
     - **Degrade silently.** No `doctrine:` labels, or no `docs/scripts/doctrine-digest.sh` → inline nothing, say nothing, compose the dispatch exactly as without this bullet. Not a warning, not an abort. A label whose id the script rejects (stale label, non-zero exit) is skipped, mentioned once in the wave narration, never fatal.
   - the relevant `CLAUDE.md` — **repo context, no longer the doctrine channel.** It carries orientation and doctrine *pointers*; where digests are inlined they are this window's binding doctrine and the pointers cover only the remainder. Where no labels exist, the pointer is all there is, as before;
...
- MUST **push** the window's deduped `doctrine:<id>` digests into the dispatch prompt when its beads carry those labels (5c) — once per window, **at most one** full-body escalation — and MUST NOT truncate a digest to fit. MUST degrade **silently** to today's composition when there are no labels or no `doctrine-digest.sh`: no digest block, no warning, no abort.
