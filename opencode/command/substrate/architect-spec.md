...
Drives the SDD spec-drafting flow end-to-end at the primary-agent level. This command itself runs Q&A with the user, discovers doctrines, dispatches `doctrine-architect` subagents in parallel, mediates cross-doctrine dependencies, composes the spec, and writes it.

**Why the orchestration runs here, and why `doctrine-architect` can't fan out.** This command body runs at the **primary-agent level** and dispatches `doctrine-architect` subagents via the Task tool. The `doctrine-architect` agent is declared with `permission.task: deny`, so it can analyze and return recommendations but **cannot spawn further subagents** — a one-level depth model. That means the fan-out must originate here, at the top level, rather than inside an intermediate orchestrator subagent (which, being a subagent itself, could not spawn the `doctrine-architect` children). Keep orchestration at the primary-agent level so it can spawn N `doctrine-architect` children directly. Mirrors how `/substrate/migrate` already works. The executing agent needs `permission.task: allow`.

## Arguments

`$ARGUMENTS` — path to a brief at `docs/tasks/ongoing/<feature>/<feature>-brief.md`.
...
Confirm the brief path was passed in `$ARGUMENTS`. If not, ask for it.
...
For each relevant doctrine, dispatch its declared `specialist` (default `doctrine-architect`) via the **Task tool** — one task per relevant doctrine, issuing all tasks in a single message (parallel where the runtime supports it). If the runtime serializes Task calls, dispatch sequentially — correctness over wall-clock; log that you're doing so. Pass each architect:
...
Each architect returns structured recommendations per the `doctrine-architect` agent's output format.

### Step 6 — Mediate cross-doctrine dependencies

If an architect flags a cross-doctrine dependency, mediate: dispatch the foreign doctrine's architect (again via the Task tool) with the dependency as additional context. Repeat until no unresolved cross-doctrine dependencies remain.
...
Before returning control, decompose the spec into a directed acyclic graph of beads so the work can be executed in parallel. As the final step, **invoke the sibling `/substrate/graph-spec` command on the spec you just wrote**, in **this same session** (both run at the primary-agent level):

```
/substrate/graph-spec docs/tasks/ongoing/<feature>/<feature>-spec.md
```

If the runtime can't invoke a command mid-session, **perform the graph-spec procedure directly** on that spec path — `graph-spec` is installed as a sibling command you can read and follow. Either path is correctness-equivalent.

That procedure parses the Prompt Execution Strategy, turns each step into a bead, infers `blocked-by:` edges, cycle-checks, and persists an epic + child beads under the canonical label `epic:<feature>` — then prints the wave shape via `docs/scripts/bead-graph.sh`. If the project has no bead-tracker configured, `graph-spec` falls back to markdown beads; either way the user sees the graph before handoff.

**This step is automatic, not optional.** Do NOT ask the user whether to graph the spec — always run `graph-spec` immediately after writing the spec. The bead approval prompt lives *inside* `graph-spec` (it previews the beads and asks before persisting); architect-spec's job is simply to always run it. The **only** acceptable non-graph outcome is `graph-spec` itself REFUSING for a genuine reason (no tracker AND the user declines markdown beads inside graph-spec's own gate) — in that case, note it and continue to the handoff. Never skip the invocation to "save a step."
