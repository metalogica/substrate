...
**Heads up:** this is a multi-minute operation. It discovers the project's doctrines, dispatches one `doctrine-architect` subagent per relevant doctrine in parallel, then executes many file edits with verification gates between each sub-step. Narrate progress so the user sees what's happening.

**SUBSTRATE_ROOT:** migrate reaches into the substrate source tree for helper scripts (`patch-convex-tsconfig.sh`, `setup-clerk.sh`). OpenCode has no plugin cache to discover that tree, so migrate reads `SUBSTRATE_ROOT` from the environment and fails fast if it is unset (see Step 1).

**Task permission:** the executing agent must have `permission.task: allow` to dispatch subagents. The `doctrine-architect` subagent runs with `permission.task: deny` — it analyzes and returns; it cannot fan out further (matching the depth model).
...
First, resolve `SUBSTRATE_ROOT`. OpenCode has no plugin cache to path-search, so migrate reads it from the environment and fails fast if unset — before any destructive work:

```bash
: "${SUBSTRATE_ROOT:?Set SUBSTRATE_ROOT to your substrate clone; OpenCode has no plugin cache to discover it.}"
test -f "$SUBSTRATE_ROOT/scripts/setup-clerk.sh" \
  || { echo "SUBSTRATE_ROOT set but $SUBSTRATE_ROOT/scripts/setup-clerk.sh is missing — point it at a substrate clone."; exit 1; }
```
...
Then map the prototype's shape before dispatching architects. Use the glob + read tools:
...
For each discovered doctrine, dispatch its declared `specialist` (default `doctrine-architect`) via the **Task tool** — issue **one Task per doctrine in a single assistant message** so they run in parallel where the runtime supports it. If the runtime serializes Task calls, fall back to dispatching them **sequentially**: correctness over wall-clock, and log the degradation so the user knows the fan-out ran serially. Each task gets the migration-focused prompt below, parameterized by the doctrine's path:
...
If the user says `n` or `modify`, iterate: ask what to change, re-dispatch the relevant `doctrine-architect`(s) via the Task tool with the change as additional context, regenerate the plan. Do NOT proceed without explicit approval.
...
Do NOT auto-launch `pnpm app:dev` — background dev servers linger after the session ends, obscure Vite logs, collide on port 5173 across repeated runs, and surprise the user. Let them start the server themselves.
...
- MUST dispatch all `doctrine-architect`s via the **Task tool** — one Task per relevant doctrine, issued in a single assistant message so they run in parallel where the runtime supports it. If the runtime serializes Task calls, fall back to sequential dispatch and log the degradation; correctness is unaffected either way. The executing agent needs `permission.task: allow`; each `doctrine-architect` runs with `permission.task: deny` and cannot fan out further.
...
- MUST NOT auto-launch `pnpm app:dev`. Background dev servers cause process-management issues (lingering after the session exits, port collisions, hidden logs). Print the commands and let the user retain control.
