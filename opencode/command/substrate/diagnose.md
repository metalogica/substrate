...
`$ARGUMENTS` — at minimum the error message. Optionally any of:
...
If only an error message is given, this command auto-collects supporting signals (git status, recent commits, compile output if reproducible). If no arguments are passed, ask the user for at least the error message and end the question with `[type 'default' to let me decide sensible defaults]` — `default` means "scan recent compile/test output and treat the most recent failure as the target."
