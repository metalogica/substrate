...
`$ARGUMENTS` — path to a spec at `docs/tasks/ongoing/<feature>/<feature>-spec.md`.
...
**3b. Execute.** Carry out the step's prompt content using the appropriate tools (edit, bash). Respect any "Tools to use" / "Tools to NOT use" hints in the step body.
...
3. **Re-run the gate on the integrated tip** (`substrate.yaml` → `gate.{compile,test,lint}`). Green ⟹ the mutation was ratify-only and lands with the feature in the same commit. Red ⟹ not ratify-only; revert the doctrine edit and defer.
...
- MUST pause for user approval between phases (after each gate). Non-negotiable — gated execution is the whole point of this command.
