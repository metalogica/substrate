...
- **invalidated tests (reconcile)** — existing tests that assert behavior this bead *changes* (distinct from new tests the step adds). A behavior change to an existing symbol / contract / exported member / required-dependency almost always breaks a test that pinned the old behavior, yet the step body rarely names it. **Reverse-scan the test tree** for tests referencing the changed symbol/file/contract and fold each matching test file into this bead's **creates** (write-scope) with a one-line `reconcile:` note. Skipping this is the single most common cause of a mid-run stall — the test lives outside the bead's Files, so the group-runner can't touch it and the break surfaces only at the integrated re-gate. Purely-additive beads have no invalidated tests — leave empty.
- **gate** — the step's Verify commands, inlined (a subagent runs these; per the parallel-execution doctrine it never touches tbd or git). Compare to `substrate.yaml`'s `gate.{compile,test,lint}`: if the bead's gate is a **strict subset** of `gate.*`, or omits a suite its layer is covered by (e.g. a frontend bead running only `tsc`, not `vitest`), tag the bead **`gate-scope: partial`** in its body — its per-bead green is a pre-check only, and the wave's **union re-gate** authorizes the merge (`agents-parallel-execution-doctrine.md §Supporting`). A gate equal to or superseting `gate.*` needs no tag.
...
The DAG is now cycle-free; cut it into agent-sized **windows** so no group-runner's context rots or
auto-compacts. Read `execution.context-budget` from `substrate.yaml` (default `0.4` if absent — a
deviatable prior, not a hard gate). Then:

1. **Estimate per-bead cost.** Walk beads in topological order. For each, estimate
   `cost = Σ(bytes of its Files/creates+consumes) + heavy-ref surcharge (schema/contract/migration
   reads) + gate-log weight + effort(XS…L)`. A heuristic prior, not a measurement — round generously.
2. **Accumulate into windows.** Open `window-1`; add beads in topological order, summing cost. When
   the running total would cross `context-budget`, close the current window and open the next.
   **Snap boundaries to file-adjacency:** never split a chain of co-edited beads (overlapping `Files`)
   across two windows — co-edited beads share a warm worktree, so they belong to the *same* window;
   file-disjoint chains fall into *separate* windows. Adjacency wins over the raw cost cut.
3. **Flag under-decomposition.** If a *single* bead's cost alone exceeds `context-budget`, do **not**
   silently over-fill: **warn** and recommend splitting that bead, then place it in its own window.
...
The primary execution door is the orchestrator — it runs this DAG as a
parallel worktree fleet over the context-budget windows:

  /substrate/orchestrate epic:<slug>

Or co-pilot a single window with gate-by-gate pauses (attended mode):
