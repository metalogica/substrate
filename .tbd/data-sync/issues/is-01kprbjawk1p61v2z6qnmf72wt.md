---
type: is
id: is-01kprbjawk1p61v2z6qnmf72wt
title: Formalize prod env-file convention (.env.prod vs vercel env pull)
kind: task
status: closed
priority: 2
version: 4
...
created_at: 2026-04-21T15:46:58.577Z
updated_at: 2026-04-21T16:49:14.159Z
closed_at: 2026-04-21T16:49:14.158Z
close_reason: null
---
...
DECISION: Option (a) — .env.prod as local staging file.

Rationale:
- .env.local stays pure-dev (already working)
- .env.prod is the single source of truth for prod values locally, gitignored
- Covers BOTH Vercel prod (VITE_*, CLERK_SECRET_KEY) AND Convex-only prod env vars
  (CLERK_JWT_ISSUER_DOMAIN, CLERK_WEBHOOK_SECRET) which Vercel doesn't hold
- Deploy skill reads .env.prod to push values to Vercel prod + Convex prod
- Tenant-ID drift detection (sub-h3i1) can diff .env.prod.CLERK_JWT_ISSUER_DOMAIN
  against Convex's stored value — straightforward comparison

Implementation:
- .gitignore pattern updated to .env* with !.env.example exception (safer than
  enumerating each .env.* variant)
- Deploy skill (sub-xz42) will walk user through populating .env.prod during
  the first prod deploy, then read from it on subsequent runs
- No .env.prod.example shipped — deploy skill handles creation interactively
