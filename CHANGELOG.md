...
- **`/substrate:dispatch` — cloud epic execution (substrate is local-only)** — the cloud door is removed: `skills/dispatch/SKILL.md`, `opencode/command/substrate/dispatch.md`, its spec + phase-1 findings under `docs/tasks/ongoing/dispatch/`, and the `references/docs-core/.github/workflows/substrate-orchestrate.yml` payload `adopt` installed. `adopt` loses its cloud-dispatch onboarding question, the `ci:`-block writer, the workflow token-substitution, the secrets handoff, and two constraints; the `ci:` block is gone from the `substrate.yaml` template (its only readers were adopt and dispatch). The parallel-execution doctrine's `## Remote / cloud orchestration (dispatch)` section is replaced by `## --pr landing (local, review-first)`. Skill count 14 → 13 across `plugin.json`, `marketplace.json`, `CLAUDE.md`, `README.md`. It was experimental, never doctrine. **Kept:** orchestrate's `--pr` flag, re-documented as a local review-first landing rather than cloud-output mode — the mechanism (push the integration branch, let GitHub squash-merge) is useful without a runner. Also kept: `doctrine-lint.yml`, the ordinary mechanical-gate CI `adopt` installs, and every ordinary use of the word "dispatch" (the orchestrator dispatching group-runners, the daemon dispatching a headless session).
